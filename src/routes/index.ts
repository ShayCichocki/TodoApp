...
router.get('/api/todos', requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
...
router.get('/api/todos/deleted', requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
...
router.get('/api/todos/:id', requireAuth, async (req: Request<{ id: string }>, res: Response): Promise<void> => {
...
router.put('/api/todos/:id', requireAuth, validateBody(UpdateTodoSchema), async (req: Request<{ id: string }>, res: Response): Promise<void> => {
  const id = parseInt(req.params.id, 10);
  ...
  try {
    const updated = await todoService.updateForUser(id, req.body, req.user!.id);
    if (!updated) {
      res.status(404).json({ error: 'Todo not found' });
      return;
    }
...
router.delete('/api/todos/:id', requireAuth, async (req: Request<{ id: string }>, res: Response): Promise<void> => {
  const id = parseInt(req.params.id, 10);
...
router.post('/api/todos/:id/restore', requireAuth, async (req: Request<{ id: string }>, res: Response): Promise<void> => {
  const id = parseInt(req.params.id, 10);
...
router.get('/api/tags', requireAuth, async (_req: Request, res: Response): Promise<void> => {
  try {
...
router.delete('/api/tags/:id', requireAuth, async (req: Request<{ id: string }>, res: Response): Promise<void> => {
  const id = parseInt(req.params.id, 10);
