...
router.get(
  '/connections',
  requireAuth,
  requireFeature('hasCalendarIntegration'),
    ...
    try {
      ...
      switch (connection.syncDirection) {
        ...
        case SyncDirection.BIDIRECTIONAL:
          result = await calendarService.performBidirectionalSync(
            id,
            req.user!.id
          );
          break;
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (
        !provider ||
        !Object.values(CalendarProvider).includes(provider as CalendarProvider)
      ) {
        res
          .status(400)
          .json({ error: 'Invalid or missing provider parameter' });
        return;
      }

      const authUrl = calendarService.getOAuthUrl(
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      await calendarService.handleOAuthCallback(
        provider,
        code as string,
        userId
      );
