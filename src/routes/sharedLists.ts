  ...
  async (req: Request<{ id: string }>, res: Response): Promise<void> => {
    ...
    try {
      ...
      if ((error as any).code === 'P2025') {
        res
          .status(404)
          .json({ error: 'List not found or you are not the owner' });
        return;
      }
  ...
  async (req: Request<{ id: string }>, res: Response): Promise<void> => {
    ...
    try {
      ...
      if ((error as any).code === 'P2002') {
        res
          .status(409)
          .json({ error: 'User is already a collaborator on this list' });
        return;
      }
 ...
 */
router.put(
  '/:listId/collaborators/:collabId',
  requireAuth,
  async (
    req: Request<{ listId: string; collabId: string }>,
    res: Response
  ): Promise<void> => {
    ...
    try {
      ...
      if (!list || list.ownerId !== req.user!.id) {
        res
          .status(403)
          .json({ error: 'Only the owner can update collaborator roles' });
        return;
      }

      const updated = await sharedListService.updateCollaboratorRole(
        collabId,
        role
      );
      res.json(updated);
...
router.delete(
  ...
  async (
    ...
    try {
      ...
      if (!isOwner && !isSelf) {
        res.status(403).json({
          error: 'You can only remove yourself or you must be the owner',
        });
        return;
      }
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const invitations = await sharedListService.getPendingInvitations(
        req.user!.id
      );
 ...
 */
router.post(
  '/invitations/:collabId/accept',
  requireAuth,
  async (req: Request<{ collabId: string }>, res: Response): Promise<void> => {
    const collabId = parseInt(req.params.collabId, 10);
    ...
    try {
      const accepted = await sharedListService.acceptInvitation(
        collabId,
        req.user!.id
      );
      res.json(accepted);
